...
function download_link(type, link, parsing_result) {
    ...
    const action = (0, child_process_1.spawn)("yt-dlp", params);
    ...
    action.on("close", code => {
        ...
        if (type == accepted_types.video)
            extrct_link(parsing_result);
    });
}
function extrct_link(parsing_result) {
    ...
    if (video_name != undefined) {
        ...
        const action = (0, child_process_1.spawn)("ffmpeg", params);
        action.stdout.on("data", data => {
            console.log(`stdout: ${data}`);
        });
        action.stderr.on("data", data => {
