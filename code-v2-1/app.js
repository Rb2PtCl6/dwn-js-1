...
function get_file_name(video_id, location) {
...
function download_link(type, link, parsing_result) {
    ...
    action.on("close", code => {
        ...
        if (type == accepted_types.video) extract_link(parsing_result)
    })*/
}
function extract_link(type, parsing_result) {
...
function delete_original(type, path) {
    if (type == accepted_types.video)
        fs.unlinkSync(path);
}
function execute_command(type, command, params, next, func) {
