...
const command = { "yt-dlp": extract_link, "ffmpeg": delete_original }
...
function download_link(type: accepted_types, link: string, parsing_result: string) {
    ...
    execute_command(type , "yt-dlp", params, parsing_result, command["yt-dlp"])
}

function extract_link(type: accepted_types, parsing_result: string) {
    var video_name = get_file_name(parsing_result, "video")
    if (video_name != undefined) { 
        ...
        execute_command(type, "ffmpeg", params, video_name_extended, command["ffmpeg"])
    }
}

function delete_original(type: accepted_types, path: string) {
    if (type == accepted_types.video) fs.unlinkSync(path)
}

function execute_command(type: accepted_types, command: string, params: string[], next: string, func: (type: accepted_types, next : string) => void) {
    ...
    action.on("close", code => {
        ...
        if (type == accepted_types.video)  func(type, next)
    })
}
